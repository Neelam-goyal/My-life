...
@EndUserText.label: 'Stmt. of Specific Emp. in Specific Comp.'
define view entity ZC_EmpStatement
 with parameters
    pCompanyCode : bukrs,
    pEmpCode   : kunnr,
    pGLAccount   : saknr,
    pFromDate    : budat,
    pToDate      : budat,
    pBusinessPlace : abap.char(4),
    @Consumption.defaultValue: 'N'
    pIsRevDoc    : abap.char(1),
    @Consumption.defaultValue: 'N'
    pIsExcSplGL    : abap.char(1)
  as select from ZR_EmpStatement as item
{
  ...
  key cast('000'as abap.numc(3))                                                                                               as AccountingDocumentItem,
      ...
      $parameters.pEmpCode                                                                                                     as EmpCode,
      EmpName,
...
where
  ...
  and item.EmpCode     = $parameters.pEmpCode
  ...
  and case when $parameters.pBusinessPlace is initial or  right($parameters.pBusinessPlace,2) = '01'
      then 1
      when  item.BusinessPlace = $parameters.pBusinessPlace then 1
      else 0 
      end = 1  
group by
  item.CompanyCode,
...
select from ZR_EmpStatement as item
{
  key CompanyCode,
