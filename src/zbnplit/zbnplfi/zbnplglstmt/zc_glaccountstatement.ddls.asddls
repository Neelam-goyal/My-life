...
@EndUserText.label: 'GL Account Statement (Opening and Statement)'

define view entity ZC_GLAccountStatement
  with parameters
    pCompanyCode : bukrs,
    pGLAccount   : saknr,
    pFromDate    : budat,
  ...
  as select from ZR_GLAccountStatement as item
{
  key item.CompanyCode,
  key cast('0000'as abap.numc(4))                                                                                              as FiscalYear,
  key $parameters.pFromDate                                                                                                    as PostingDate,
  key cast('0000000000' as abap.char(10))                                                                                      as AccountingDocument,
  key cast('000'as abap.numc(3))                                                                                               as AccountingDocumentItem,
  
      $parameters.pGLAccount                                                                                                   as GLAccount,
      GLAccountName,

      cast('0000000000' as abap.char(10))                                                                                      as Supplier,
      cast('0000000000' as abap.char(10))                                                                                      as Customer,
...
where
  ...
  and item.GLAccount   = $parameters.pGLAccount
...
select from ZR_GLAccountStatement as item
{
  key CompanyCode,
  key FiscalYear,
  key PostingDate,
