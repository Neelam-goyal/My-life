CLASS zcl_cust_supp_fiscal_stmt DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS get_data_account_statement FOR TABLE FUNCTION ztblf_account_statement.
  PROTECTED SECTION.
  PRIVATE SECTION.
ENDCLASS.
...
CLASS ZCL_CUST_SUPP_FISCAL_STMT IMPLEMENTATION.


  METHOD get_data_account_statement
         BY DATABASE FUNCTION FOR HDB
           LANGUAGE SQLSCRIPT
           OPTIONS READ-ONLY
           USING ZC_AccountStatement ZR_BillingDocumentRef ZR_PurchaseDocumentRef ZR_PaymentDocumentRef.
    RETURN
    ...
    select
          ...
          item.tdsamountincmpcdcrcy
    from
        ZC_AccountStatement( :pCompanyCode , :pCust_Supp, :pGLAccount, :pFromDate, :pToDate, :pBusinessPlace, :pIsRevDoc, :pIsExcSplGL) as item
        Left outer Join
             (
