    ...
    CLASS-METHODS :
      read_posts
        ...
        IMPORTING CompanyCode      TYPE string ##NEEDED
                  VALUE(IngoinGL)  TYPE string
                  VALUE(OutgoinGL) TYPE string
...
PROTECTED SECTION.
    ...
    DATA: wa_doc  TYPE ty_doc,
          it_item TYPE STANDARD TABLE OF ty_doc WITH EMPTY KEY.
...
CLASS ZCL_BANKSTATEMENT IMPLEMENTATION.
  ...
  METHOD read_posts.
    ...
    WHERE ( a~GLAccount = @ingoingl OR a~GLAccount = @outgoingl )
    INTO TABLE @DATA(gl_text).

    DATA : gltext TYPE string.
    LOOP AT gl_text ASSIGNING FIELD-SYMBOL(<wa_glTxt>).
      gltext = |{ <wa_glTxt>-GLAccountLongName } [{ <wa_glTxt>-GLAccount }]|.  " <-- use the field symbol
    ENDLOOP.
    ...
    IF profitcenter IS NOT INITIAL.
      SELECT FROM i_accountingdocumentjournal( P_Language = 'E' ) AS a
        ...
        WHERE a~IsReversed IS INITIAL
         ...
         and ( a~GLAccount = @ingoingl OR a~GLAccount = @outgoingl )
    ...
    SELECT FROM  i_accountingdocumentjournal( P_Language = 'E' )
        ...
        WHERE PostingDate LT @lastdate AND Ledger = '0L' AND IsReversed IS INITIAL
         ...
         and ( GLAccount = @ingoingl OR GLAccount = @outgoingl )
