CLASS zcl_batchjobcalculate DEFINITION
  PUBLIC
...
CLASS ZCL_BATCHJOBCALCULATE IMPLEMENTATION.
  ...
  SELECT FROM zc_invgrouped
  FIELDS Orderdate
  WHERE Highlight EQ 1
  GROUP BY ORDERDATE
  INTO TABLE @DATA(lt_orderdates).

  loop AT lt_orderdates into DATA(ls_orderdate).


     DATA(curDate) = ls_orderdate-Orderdate.
   ...
   ENDMETHOD.
    ...
    METHOD UpdatedDoc.
      ...
      FIELDS a~BillingDocument, ( a~TotalNetAmount + a~TotalTaxAmount ) AS TotAmt
        ...
        and a~BillingDocumentIsCancelled is INITIAL and a~CancelledBillingDocument is INITIAL
    ...
    METHOD generateForSales.
      ...
      MODIFY ENTITIES OF zr_invgrouped000
      ...
      COMMIT ENTITIES BEGIN
          RESPONSE OF zr_invgrouped000
      REPORTED DATA(committed)
