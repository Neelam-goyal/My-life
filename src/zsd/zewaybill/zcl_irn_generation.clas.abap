...
*        wa_itemlist-unit = .
     ...
     AND billingdocument = @wa_lines-billingdocument AND billingdocumentitem = @wa_lines-billingdocumentitem
      ...
      SELECT FROM i_billingdocumentitemprcgelmnt AS a
         FIELDS SUM( a~ConditionRateAmount ) AS UnitPrice, SUM( a~ConditionAmount ) AS TotAmt
         WHERE   conditiontype IN ( 'ZFOC' )
             AND billingdocument = @wa_lines-billingdocument AND billingdocumentitem = @wa_lines-billingdocumentitem
         INTO @DATA(foc_charge) .



      IF wa_itemlist-cgst_amount IS INITIAL OR foc_charge IS NOT INITIAL.
        wa_itemlist-cgst_amount = '0'.
      ENDIF.
      IF wa_itemlist-sgst_amount IS INITIAL OR foc_charge IS NOT INITIAL.
        wa_itemlist-sgst_amount = '0'.
      ENDIF.
      IF wa_itemlist-igst_amount IS INITIAL OR foc_charge IS NOT INITIAL.
        wa_itemlist-igst_amount = '0'.
      ENDIF.
      IF wa_itemlist-other_charge IS INITIAL OR foc_charge IS NOT INITIAL.
        wa_itemlist-other_charge = '0'.
      ENDIF.
      IF wa_itemlist-cess_nonadvol_amount IS INITIAL OR foc_charge IS NOT INITIAL.
