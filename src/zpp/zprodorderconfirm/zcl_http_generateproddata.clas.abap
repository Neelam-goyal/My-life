CLASS zcl_http_generateproddata DEFINITION
  ...
  PUBLIC SECTION.
    ...
    TYPES: BEGIN OF tt_response,
             ...
             OverdeliveryTolerance TYPE p LENGTH 3 DECIMALS 1,
             CreatedDate           TYPE d,
...
CLASS ZCL_HTTP_GENERATEPRODDATA IMPLEMENTATION.
  ...
  METHOD fetchdetails.
    ...
    SELECT SINGLE FROM I_ManufacturingOrder AS a
        INNER JOIN I_ProductionOrder AS f  ON f~ProductionOrder = a~ManufacturingOrder
        INNER JOIN I_ProductionOrderItem AS g ON f~ProductionOrder = g~ProductionOrder
        ...
        FIELDS b~ProductDescription,a~Product,a~ManufacturingOrder,a~ProductionPlant,e~WorkCenterText,d~WorkCenter,c~ManufacturingOrderOperation_2,
               c~ManufacturingOrderSequence,c~MfgOrderOperationText,c~OperationConfirmation,a~BillOfOperationsGroup,a~CompanyCode, f~OrderActualReleaseDate,
               g~OverdelivTolrtdLmtRatioInPct
    ...
    response_type-createddate = mfg_order_basic-OrderActualReleaseDate.
    response_type-confirmationtext = request_data-confirmationtext.
    response_type-overdeliverytolerance = mfg_order_basic-OverdelivTolrtdLmtRatioInPct.
...
*   Get Activities and Yield Quantities
    ...
    LOOP AT lt_confproposal INTO DATA(conf_proposal).
      response_type-yieldquantity = COND #(
                                        WHEN ( mfg_order_basic-CompanyCode = 'BBPL' OR mfg_order_basic-CompanyCode = 'HOVL' )
                                             AND request_data-YieldQuantity IS INITIAL
                                          THEN 0
                                        ELSE conf_proposal-%param-ConfirmationYieldQuantity
                                    ).
...
*      INTO @DATA(bill_of_material).
    ...
    SELECT FROM I_MfgOrderComponentWithStatus AS a
        INNER JOIN I_Product AS c ON a~Material = c~Product
        INNER JOIN I_ProductDescription_2 AS b ON a~Material = b~Product
        FIELDS a~MaterialComponentIsPhantomItem, a~Material, a~Plant, a~StorageLocation, a~Batch, a~GoodsMovementType, a~EntryUnit,b~ProductDescription,
               c~ProductType,a~BillOfMaterialItemNumber,a~GoodsMovementEntryQty AS RequiredQuantity, a~MaterialQtyToBaseQtyNmrtr, a~MaterialQtyToBaseQtyDnmntr
        WHERE ManufacturingOrder = @mfg_order_basic-ManufacturingOrder
        AND a~MaterialComponentIsPhantomItem <> 'X'
...
*      IF bom_item IS INITIAL.
      IF mfg_order_component-RequiredQuantity IS INITIAL .
        multiplier = 0.
...
*        multiplier =  bom_item.
      ELSE.
        multiplier =  mfg_order_component-RequiredQuantity / mfg_order_item_status-MfgOrderItemPlannedYieldQty. "* ( 1 + ( ComponentScrapInPercent / 100 ) ).
*        multiplier =  mfg_order_component-RequiredQuantity / bill_of_material-BOMHeaderQuantityInBaseUnit * ( 1 + ( ComponentScrapInPercent / 100 ) ).
      ENDIF.
...
************** changes made by mrityunjay for decimal places
      ...
      LOOP AT stock_with_batch INTO DATA(stock_item).
        ...
        stock_item-MatlWrhsStkQtyInMatlBaseUnit = stock_item-MatlWrhsStkQtyInMatlBaseUnit * mfg_order_component-MaterialQtyToBaseQtyDnmntr /
                                               mfg_order_component-MaterialQtyToBaseQtyNmrtr.
    ...
    REPLACE ALL OCCURRENCES OF '"OVERDELIVERYTOLERANCE"' IN lv_string WITH '"OverdeliveryTolerance"'.
...
*************validation so that confirmed qty + overdelivery within tol limit

*      select single from I_MfgOrderWithStatus as a
*      left join I_ProductionOrderItem as b on a~ManufacturingOrder = b~productionorder
*      fields
*      a~MfgOrderConfirmedYieldQty,
*      b~OVERDELIVTOLRTDLMTRATIOINPCT
*       WHERE a~ManufacturingOrder = @filled_details-ManufacturingOrder
*      INTO @DATA(ls_mfg_check).
    ...
    ELSEIF mfg_order-OrderIsDelivered IS NOT INITIAL.
      ...
      RETURN.
*    elseif ls_mfg_check IS NOT INITIAL.
*      IF ls_mfg_check-MfgOrderConfirmedYieldQty + ls_mfg_check-OverDelivTolrtdLmtratioInPct = ls_mfg_check-MfgOrderConfirmedYieldQty.
*      message = |Manufacturing Order { filled_details-ManufacturingOrder } is already delivered.|.
*      RETURN.
*      ENDIF.
